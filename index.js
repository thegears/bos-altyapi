  ...
  client.on("message", async (message) => {
    ...
    if (
      ...
      return message.reply(
        `Benim prefixim: **\`${prefix}\`**, yaz \`${prefix}help\` bilgi için!`
      ); //if the messages is not a command and someone tags the bot, then send an info message
    ...
    if (command) {
      ...
      if (timestamps.has(message.author.id)) {
        ...
        if (now < expirationTime) {
          ...
          return message.reply(
            `\`${command.name}\` komutunu kullanmadan önce  ${timeLeft.toFixed(
            1
          )} saniye bekleyin.  `
      ...
      try {
        ...
        console.log(error);
        return message.reply(
          "Bu komutu çalıştırırken bir şeyler ters gitti: `" +
          command.name +
          "` komut"
        );
      }
